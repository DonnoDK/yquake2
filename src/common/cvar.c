...
char * Cvar_CompleteVariable(char *partial) {
	...
	/* check exact match */
    cvar_t* cvar = Cvar_FindVar(partial);
    if(cvar){
        return cvar->name;
    }
 ...
 */
void Cvar_Set_f(int argc, const char** argv) {
	if ((argc != 3) && (argc != 4)) {
	...
	if (argc == 4) {
		...
		Cvar_FullSet(argv[1], argv[2], flags);
	} else {
...
static void Cvar_List_f(int argc, const char** argv) {
	int i = 0;
	for (cvar_t* var = cvar_vars; var; var = var->next, i++) {
        Com_Printf(var->flags & CVAR_ARCHIVE ?    "*" : " ");
        Com_Printf(var->flags & CVAR_USERINFO ?   "U" : " ");
        Com_Printf(var->flags & CVAR_SERVERINFO ? "S" : " ");
        Com_Printf(var->flags & CVAR_NOSET ?      "-" : var->flags & CVAR_LATCH ? "L" : " ");
		Com_Printf(" %s \"%s\"\n", var->name, var->string);
	}
	Com_Printf("%i cvars\n", i);
}
...
void Cvar_Init(void)
{
	Cmd_AddDelegate("set", Cvar_Set_f);
	Cmd_AddDelegate("cvarlist", Cvar_List_f);
}
