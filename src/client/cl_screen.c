...
void
DrawHUDStringScaled(const char *string, int x, int y, int centerwidth, int xord, float factor)
{
	int margin;
	char line[1024];
	int width;
	...
	while (*string)
	{
		...
		for (i = 0; i < width; i++)
		{
			Draw_CharScaled(x, y, line[i] ^ xord, factor);
			x += 8*factor;
		}
...
void
DrawHUDString(char *string, int x, int y, int centerwidth, int xord)
{
	DrawHUDStringScaled(string, x, y, centerwidth, xord, 1.0f);
}
...
void SCR_UpdateScreen(void) {
	...
	for (i = 0; i < numframes; i++) {
        ...
        /* NOTE (brian): are we showing loading screen? */
        ...
        /* NOTE (brian): are we showing cinematic? */
        ...
        /* NOTE (brian): are we just doing normal game rendering? */
        else {
            ...
            /* TODO: remove this entirely, as soon as screen-resizing has been removed */
			SCR_TileClear();
